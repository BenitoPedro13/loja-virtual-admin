...
import { PermissaoService } from "../../../demo/service/cadastro/BaseService";

const CrudPermissao = () => {
  let emptyObject: Demo.Permissao = {
    nome: "",
  };

  const [objects, setObjects] = useState<Demo.Permissao[]>([]);
  ...
  const [object, setObject] = useState<Demo.Permissao>(emptyObject);
  const [selectedObjects, setSelectedObjects] = useState<Demo.Permissao[]>([]);
  const [submitted, setSubmitted] = useState(false);
  const [globalFilter, setGlobalFilter] = useState("");
  const toast = useRef<Toast>(null);
  const dt = useRef<DataTable<Demo.Permissao[]>>(null);

  useEffect(() => {
    if (objects.length === 0) {
      PermissaoService.getAll().then((data) => (data ? setObjects(data) : null));
    }
  }, [objects]);
  ...
  const saveProduct = async () => {
    ...
    if (object.nome.trim()) {
      ...
      if (object.id) {
        ...
        await PermissaoService.update(_object)
          .then((data) => {
            if (!data) {
              return toast.current?.show({
                severity: "error",
                summary: "Erro",
                detail: "Erro ao Atualizar Permissao",
                life: 3000,
              });
            }
          ...
          .catch((error) => {
            ...
            return toast.current?.show({
              severity: "error",
              summary: "Erro",
              detail: `Erro ao Atualizar Permissao: ${error}`,
              life: 3000,
            });
          });
      } else {
        await PermissaoService.create(_object)
          .then((data) => {
            if (!data) {
              return toast.current?.show({
                severity: "error",
                summary: "Erro",
                detail: "Erro ao Criar Permissao",
                life: 3000,
              });
            }
          ...
          .catch((error) => {
            ...
            return toast.current?.show({
              severity: "error",
              summary: "Erro",
              detail: `Erro ao Criar Permissao: ${error}`,
              life: 3000,
            });
          });
      }
  ...
  const editProduct = (product: Demo.Permissao) => {
    setObject({ ...product });
    setObjectDialog(true);
  };

  const confirmDeleteProduct = (product: Demo.Permissao) => {
    setObject(product);
  ...
  const deleteProduct = async () => {
    ...
    if (_object.id) {
      ...
      await PermissaoService.delete(_object.id)
        .then((ok) => {
          if (index !== -1 && ok) {
            ...
            toast.current?.show({
              severity: "error",
              summary: "Erro",
              detail: "Erro ao Deletar Permissao.",
              life: 3000,
            });
          }
        })
        .catch((error) => {
          ...
          return toast.current?.show({
            severity: "error",
            summary: "Erro",
            detail: `Erro ao Deletar Permissao: ${error}`,
            life: 3000,
          });
        });
      setObjects(_objects);
  ...
  const deleteSelectedProducts = async () => {
    ...
    try {
      // Call the bulkDelete method of PermissaoService
      await PermissaoService.bulkDelete(selectedProductIds).then((ok) => {
        if (ok) {
          ...
          toast.current?.show({
            severity: "error",
            summary: "Erro",
            detail: "Erro ao Deletar Permissaos.",
            life: 3000,
          });
        }
      });
      ...
      return toast.current?.show({
        severity: "error",
        summary: "Erro",
        detail: `Erro ao Deletar Permissaos: ${error}`,
        life: 3000,
      });
    }
  ...
  const codeBodyTemplate = (rowData: Demo.Permissao) => {
    return (
      <>
  ...
  const nameBodyTemplate = (rowData: Demo.Permissao) => {
    return (
      <>
  ...
  const actionBodyTemplate = (rowData: Demo.Permissao) => {
    return (
      <>
        <Button
          icon="pi pi-pencil"
  ...
  const header = (
    <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
      <h5 className="m-0">Gerencie Permissaos</h5>
      ...
      <div className="col-12">
        <div className="card">
          ...
          <DataTable
            ...
            onSelectionChange={(e) =>
              setSelectedObjects(e.value as Demo.Permissao[])
            }
            dataKey="id"
            paginator
            rows={10}
            ...
            currentPageReportTemplate="Mostrando {first} ate {last} de {totalRecords} permissões"
            globalFilterFields={["nome", "sigla", "id"]}
            globalFilter={globalFilter}
            emptyMessage="Nenhum Permissao encontrado."
            header={header}
          ...
          >
            <div className="flex align-items-center justify-content-center">
              ...
              {object && (
                <span>
                  Tem certeza que deseja deletar os permissões selecionados ?
                </span>
              )}
            </div>
          </Dialog>
